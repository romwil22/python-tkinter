...
def next(imageNumber):
    ...
    # image number
    labelStatus = Label(imageViwerWindow, text="images " + str(imageNumber) +"/"+str(len(imageList)), bd=2, relief=SUNKEN, anchor=W)
    labelStatus.grid(row=2, column=0, columnspan=3, sticky=W+E)
...
labelStatus = Label(imageViwerWindow, text="images " + "1" +"/"+str(len(imageList)), bd=2, relief=SUNKEN, anchor=W) 
...
# buttons
backButton.grid(row=1, column=0)
exitButton.grid(row=1, column=1, pady=5)
nextButton.grid(row=1, column=2)
